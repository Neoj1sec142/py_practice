...
class LinkedList:
    ...
    def search(self, key):
        ...
        returns node or None if not found - takes O(n) - linear'''
        current = self.head
        while current:
            if current.data == key:
                return current
    ...
    def insert(self, data, index):
        '''inserts a new node at containing data at  index position
        insertion takes O(1) time but finding the node at the insertion
        point takes O(n) time therefore overall - O(n) linear time'''
        if index == 0:
            self.add(data)
        if index > 0:
            new = Node(data)
